...
export class EmailService {
   ...
   */
  static async verifyConnection(config: SmtpConfig): Promise<{ success: boolean; message: string }> {
   ...
   * Send Test Email
   */
  static async sendTestEmail(config: SmtpConfig, toEmail: string): Promise<{ success: boolean; message: string }> {
    if (!toEmail) {
   ...
   */
  static async sendAnniversaryEmail(options: SendEmailOptions): Promise<{ success: boolean; message: string }> {
   ...
   */
  private static generateHtmlTemplate(event: any, calc: any): string {
...
<body style="margin: 0; padding: 30px 15px; background-color: #FDF8F3; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', 'Microsoft YaHei', sans-serif; color: #2D2424; -webkit-font-smoothing: antialiased;">
   ...
   * Plain text fallback
   */
  private static generatePlainText(event: any, calc: any): string {
    const isToday = calc.daysRemaining === 0;
