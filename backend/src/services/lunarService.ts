...
export interface EventData {
  ...
  is_leap_month?: boolean | number;
  repeat_type: string;       // 'none' | 'year' | 'month' | 'week' | 'custom_days'
  repeat_interval?: number;
  repeat_weekdays?: string;  // e.g. '[1, 5]'
  direction?: string;        // 'countdown' | 'accumulate' | 'both'
}

export interface CalculationResult {
  nextDate: string;          // YYYY-MM-DD
  daysRemaining: number;     // 0 = today, >0 = future
  isToday: boolean;
  daysPassed: number;        // Total days since target_date
  yearsCount: number;        // How many years since target_date
...
export function calculateEvent(event: EventData, todayStr?: string): CalculationResult {
  const now = new Date();
  ...
  if (isLunar) {
    try {
      const origLunar = Lunar.fromYmd(origY, origM, origD);
    ...
    try {
      const origSolar = Solar.fromYmd(origY, origM, origD);
  ...
  if (repeatType === 'none') {
    if (isLunar) {
      try {
        const lunar = Lunar.fromYmd(origY, origM, origD);
    ...
    if (!isLunar) {
      ...
      let foundSolar: string | null = null;
      for (let y = curY; y <= curY + 3; y++) {
        try {
          ...
          const lunarCandidate = isLeap ? Lunar.fromYmd(y, -origM, origD) : Lunar.fromYmd(y, origM, origD);
          ...
          try {
            const lunarFallback = Lunar.fromYmd(y, origM, origD);
    ...
    if (!isLunar) {
      ...
      let foundSolar: string | null = null;
      const curSolarObj = Solar.fromYmd(curY, curM, curD);
      ...
      for (let i = 0; i < 12; i++) {
        try {
          const lunar = Lunar.fromYmd(lY, lM, origD);
    ...
    if (curDayOfWeek === 0) curDayOfWeek = 7; // Convert Sunday to 7
  ...
  const nextSolar = Solar.fromYmd(nY, nM, nD);
  ...
  const isBirthday = 
    (typeof event.category_name === 'string' && (event.category_name.includes('生日') || event.category_name.includes('生'))) ||
    (typeof event.title === 'string' && (event.title.includes('生日') || event.title.includes('生')));

  const birthLunar = origY ? Lunar.fromYmd(origY, 1, 1) : nextLunar;
...
import { SOLAR_TERMS_MAP, SolarTermDetail } from './solarTermsData';

export function getCurrentSolarTermInfo(date?: Date) {
  ...
  const detail: SolarTermDetail = SOLAR_TERMS_MAP[prevName] || SOLAR_TERMS_MAP['秋分'];
